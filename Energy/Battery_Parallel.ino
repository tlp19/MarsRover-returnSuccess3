    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 1:{ //Relay is high, measure the OCV, NEED TO REMOVE RESTRICTION ON V BETWEEN 3700 AND 2400
        ...
        if(Relay_Counter < 100){
          ...
          if(SoC1_Approx>SoC2_Approx){//Cell 1 has higher charge than Cell 2
            if((SoC1_Approx-SoC2_Approx)<10){
              digitalWrite(4,false);
              digitalWrite(5,false);
              next_state = 3;
            }else{
              digitalWrite(5,false); // Need to charge Cell 2
              next_state = 2;
            }
          }else{
            if((SoC2_Approx-SoC1_Approx)<10){
              digitalWrite(4,false);
              digitalWrite(5,false);
              next_state = 3;
            }else{
      ...
      case 6:{ // Charge Rest, green LED is off and no current
        current_ref = 0;
      ...
      case 7:{ //Discharge state (-250mA and no LEDs)
         current_ref = -500;
      ...
      case 8:{ // Discharge rest, no LEDs no current
        current_ref = 0;
        if (rest_timer < 30) { // Rest here for 30s like before
          next_state = 8;
          digitalWrite(8,false);
          rest_timer++;
        } else { // When thats done, move back to passive balancing (and light the green LED)
          next_state = 3;
          digitalWrite(8,true);
          rest_timer = 0;
        }
      ...
      case 9: { // ERROR state RED led and no current
        current_ref = 0;
        next_state = 9; // Always stay here
        digitalWrite(7,true);
        digitalWrite(8,false);
        if(input_switch == 0){ //UNLESS the switch = 0, then go back to start
          next_state = 0;
      ...
      default :{ // Should not end up here ....
        Serial.println("Boop");
        current_ref = 0;
        next_state = 9; // So if we are here, we go to error
        digitalWrite(7,true);
      }
    }

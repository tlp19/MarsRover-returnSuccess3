...
unsigned int upperindex, lowerindex; //upper and lower index for the OCV-SoC graph
...
float Capacitance_Max = 586.5, Capacitance_Consumed;
float Current_Counter = 0;
float OCV; //Open Circuit Voltage
float Constraint = 100;
float upperpoint, lowerpoint; //upper and lower OCV with the upper and lower indexes
float SoC_Upper, SoC_Lower, SoC_Approx; //Approximation of SoC with function
...
void loop() {
  if (loop_trigger == 1){ // FAST LOOP (1kHZ)
    ...
    if(state_num == 2){
        //Initialise Array
        float OCV_Array[] = {3427.91,3332.47,3332.47,3336.45,3332.47,3328.5,3320.54,3308.61,3300.66,3300.66,3296.68,3296.68,3292.71,3288.73,3276.8,3264.87,3248.96,3225.1,3209.2,3125.69};
        ...
        upperpoint = OCV_Array[upperindex];
        lowerpoint = OCV_Array[lowerindex];
        SoC_Upper = SoC_Array[upperindex];
        SoC_Lower = SoC_Array[lowerindex];
        
        if((upperpoint - lowerpoint) == 0){
          SoC_Approx = SoC_Upper - SoC_Lower;
        }else{
          SoC_Approx = InitialSoC(upperindex,SoC_Upper,OCV,upperpoint, lowerpoint);
        }
    }
  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz)
    ...
    dataString = String(state_num) + "," + String(V_Bat) + "," + String(current_ref) + "," + String(current_measure) + "," + String(OCV) +","+String(Relay_Counter) +"," + String(upperindex) +","+ String(lowerindex)+"," +String(upperpoint) +","+ String(lowerpoint)+","+ String(SoC_Upper)+","+String(SoC_Lower)+","+String(SoC_Approx); //build a datastring for the CSV file
